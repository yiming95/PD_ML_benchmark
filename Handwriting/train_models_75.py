...
N_EPOCHS = 30
RANDOM_SEED = 92
PREDICTIONS_FOLDER = Path('models/preds/75_25/')
...
def get_parameters() -> argparse.Namespace:
    ...
    parser.add_argument('--ratio', help='Either 75_25 or 75_25', type=str, required=True)
...
def train_models(exam: str,
    ...
    for model_no in range(1, n_models + 1):
        ...
        model_name = 'amodel_num_' + str(model_no) + "_" + time_name.split(" ")[0] + "-" + \
                     time_name.split(" ")[1].split(".")[0].replace(':', '-')
        log_dir = f'./logs/75_25/{model_name}'
        cbs = [
            callbacks.ModelCheckpoint(f'trained_models/75_25/{model_name}',
                                      save_best_only=True,
                                      verbose=1),
            callbacks.TensorBoard(f'./logs/75_25/{model_name}'),
        ...
        # Evaluating
        best_model = models.load_model(f'trained_models/75_25/{model_name}',
        ...
        df.to_csv("./csvresults/75_25/" + model_name + '.csv', index=False)
