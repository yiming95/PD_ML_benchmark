...
N_EPOCHS = 30
RANDOM_SEED = 92
PREDICTIONS_FOLDER = Path('models/preds/mea_75_25/')
...
def train_models(exam: str,
    ...
    for model_no in range(1, n_models + 1):
        ...
        log_dir = f'./logs/mea_75_25/{model_name}'
        cbs = [
            callbacks.ModelCheckpoint(f'trained_models/mea_75_25/{model_name}',
                                      save_best_only=True,
                                      verbose=1),
            callbacks.TensorBoard(f'./logs/mea_75_25/{model_name}'),
        ...
        # Evaluating
        best_model = models.load_model(f'trained_models/mea_75_25/{model_name}',
        ...
        df.to_csv("./csvresults/mea_75_25/" + model_name + '.csv', index=False)
